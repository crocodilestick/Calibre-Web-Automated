...
class Amazon(Metadata):
    ...
    def _parse_published_date(self, container: BS) -> str | None:
        try:
            pub_date_text = (
                container.find(
    ...
    def parse_detail_page(
        ...
        match = MetaRecord(
            ...
            publishedDate=self._parse_published_date(container),
    ...
    def parse_search_results(self, search_results: BS) -> list[str]:
        links: list[str] = []
    ...
    def search(
        ...
        if search_results_page is None:
            log.warning(f"Could not fetch {self.__name__} search results for query: {query}")
            return []
