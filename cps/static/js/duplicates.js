...
$(document).ready(function () {
    ...
    if ($(this).hasClass("disabled")) {
      ...
      if (selectedBooks.length < 2) {
        $("#error_modal_message").text(
          "Please select at least 2 books to merge."
        );
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.books && response.books.length > 0) {
            $("<span>✓ " + response.books[0] + "</span>").appendTo(
              "#display-merge-target-book"
            );

            // Rest are source books (merged and deleted)
            for (var i = 1; i < response.books.length; i++) {
              $("<span>- " + response.books[i] + "</span><p></p>").appendTo(
                "#display-merge-source-books"
              );
            }
          }
        },
        error: function (xhr, status, error) {
          $("#error_modal_message").text(
            "Error loading book list for merge confirmation. Status: " +
              xhr.status +
  ...
  $("#merge_selected_confirm").click(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          $("#success_modal_message").text(
            "Selected books have been merged successfully!"
          );
          ...
          $("#error_modal_message").text(
            "Error: " +
              (response.error || "Unknown error occurred during merge")
          );
          $("#error_modal").modal("show");
        }
      },
      error: function (xhr, status, error) {
        ...
        // Show error modal
        $("#error_modal_message").text(
          "An error occurred while merging books. Check browser console for details."
        );
        $("#error_modal").modal("show");
      },
    });
  ...
  $(document).on("click", ".dismiss-duplicate-btn", function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          if (action === "dismiss") {
            ...
            btn.html(
              '<span class="glyphicon glyphicon-eye-close"></span> Dismiss'
            );
      ...
      error: function (xhr, status, error) {
        console.error(
          "[CWA Duplicates] Error " + action + "ing duplicate group:",
          error
        );
