...
class TestAmazonMetadataProviders(MetadataProviderTestBase):
    ...
    def test_amazon_search_results(self) -> None:
        ...
        assert results == ["/ssdp/ABCDEF", "/dp/ABCDEF", "/dp/GHIJKL"]

    def test_amazon_search_returns_empty_list_when_search_page_fetch_fails(self) -> None:
        """Test that search gracefully handles a failed search results fetch."""
        provider = Amazon()
        provider.get = MagicMock(return_value=None)

        assert provider.search("test query") == []

    def test_amazon_parse_detail_page_returns_none_for_missing_container(self) -> None:
        """Test that malformed detail pages are ignored instead of raising exceptions."""
        provider = Amazon()
        provider.get = MagicMock(
            return_value=BS("<html><body><div>No product container</div></body></html>", "lxml")
        )

        assert provider.parse_detail_page("/dummy-url", "/generic.jpg", 1) is None
